...
Phytoplankton blooms along the Romagna coast (northern Adriatic Sea) are driven primarily by the nutrient load of the Po river, and their monitoring supports water-quality management. This report presents an end-to-end, reproducible geospatial system that estimates bloom risk from freely available satellite and hydro-meteorological data, serves the results through a spatial database and a web map, and quantifies the uncertainty of every prediction. On top of the predictive layer, a causal analysis estimates the effect of Po discharge on coastal chlorophyll-a, adjusted for seasonal and meteo-marine confounders, using a transparent regression and the DoWhy framework with standard refutation tests. On six seasons (2018–2023) of data over five demonstrative coastal cells, a gradient-boosting model does not outperform a persistence baseline at daily resolution, but 7-day-lagged Po discharge consistently emerges as a leading driver; the causal analysis estimates an effect of approximately +3.1 mg/m³ of chlorophyll-a per +1000 m³/s of Po discharge, robust to refutation tests but subject to unobserved confounding and temporal autocorrelation. The contribution is not methodological novelty but the integration, at fine geographic scale and in an operational, uncertainty-aware form, of methods that are individually established.
...
The causal question is the effect of (7-day-lagged) Po discharge on chlorophyll, adjusted for confounders. Two complementary approaches are used. **(A) Transparent estimate:** ordinary regression with and without confounders (season, SST), so that the change in the coefficient makes the confounding explicit. **(B) DoWhy:** an explicit DAG (confounders → treatment and → outcome; stratification declared as a mediator and not adjusted for the total effect), formal identification of the back-door adjustment set, estimation, and three refutation tests (placebo treatment, random common cause, data subset).
...
**Causal effect.** The raw correlation between lagged Po and chlorophyll is r ≈ 0.42. The apparent effect (no controls) is +3.34 mg/m³ per +1000 m³/s; adjusting for season and SST (Tappa A) gives +3.18; the DoWhy estimate adjusting for season, SST and wind (Tappa B) gives +3.14. Seasonal and thermal confounders thus explain only ≈ 5% of the apparent effect. Refutation tests behave as expected: placebo effect +0.02 (≈ 0), random common cause +3.14 (unchanged), data-subset +3.13 (stable). Over the observed discharge range (~460–3970 m³/s), the implied effect is of order 10 mg/m³.
...
- **Scale and form.** Five demonstrative cells over six seasons, with a linear causal specification, make the estimate indicative rather than definitive.
