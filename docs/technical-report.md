...
- **SST data coverage.** The nearest-pixel SST retrieval is valid for only 2 of the 5 cells across all six seasons in this reprocessed product; the other three consistently fall on a masked/land pixel, a systematic issue rather than missingness at random. Analyses that exclude SST as a confounder (B, D) therefore use the full 5-cell sample, while those that include it (A, C) are estimated on a smaller effective sample.
