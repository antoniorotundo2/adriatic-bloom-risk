...
**Author:** Antonio Rotundo — MSc student, Computer Science and Engineering, University of Bologna
...
The causal question is the effect of (7-day-lagged) Po discharge on chlorophyll, adjusted for confounders. Four complementary approaches are used. **(A) Transparent estimate:** ordinary regression with and without confounders (season, SST), so that the change in the coefficient makes the confounding explicit. **(A-bis) Fixed-effects robustness check:** a two-way fixed-effects regression (cell and year dummies, season and wind as controls) that absorbs any time-invariant per-cell trait and any year-specific shock common to all cells — a stricter, still fully transparent test, at no additional library cost. **(B) DoWhy:** an explicit DAG (confounders → treatment and → outcome; stratification declared as a mediator and not adjusted for the total effect), formal identification of the back-door adjustment set, estimation, and three refutation tests (placebo treatment, random common cause, data subset). **(C) Causal forest (EconML CausalForestDML):** rather than a single average effect, estimates whether the effect varies with distance to the Po mouth (spatial heterogeneity) and with year (temporal heterogeneity), using the same controls as (A-bis).

A methodological note applies to (A-bis) and (C): SST has valid nearest-pixel coverage for only 2 of the 5 cells across all six seasons in this reprocessed product (the other three consistently fall on a masked/land pixel — a systematic grid-coastline alignment issue, not missing at random). Including SST as a confounder would silently shrink these two analyses to those 2 cells; season and wind (complete across all 5 cells) are used instead. Steps (A) and (B), which do include SST, are consequently estimated on a smaller effective sample than (A-bis) and (C).
...
**Causal effect (average).** The raw correlation between lagged Po and chlorophyll is r ≈ 0.42. The apparent effect (no controls) is +3.34 mg/m³ per +1000 m³/s; adjusting for season and SST (A) gives +3.18; the DoWhy estimate adjusting for season, SST and wind (B) gives +3.14; the two-way fixed-effects estimate (A-bis), adjusting for season, wind, cell and year, gives +2.34 — an 8% reduction from the pooled estimate using the same controls, but still positive and statistically distinguishable from zero. The estimate narrows as controls get stricter but does not collapse to zero. Refutation tests for (B) behave as expected: placebo effect +0.02 (≈ 0), random common cause +3.14 (unchanged), data-subset +3.13 (stable). Over the observed discharge range (~460–3970 m³/s), the implied average effect is of order 10 mg/m³.

**Causal effect (heterogeneity).** The causal forest (C) finds a spatial pattern: the estimated effect decreases monotonically with distance from the Po mouth, from +1.75 mg/m³ per +1000 m³/s at Casalborsetti (90% CI +0.73, +2.77) and +1.13 at Lido Adriano (CI +0.25, +2.01), to statistically indistinguishable from zero at Cesenatico, Rimini and Cattolica (CIs spanning zero). This is directionally consistent with a physical dilution of the river's influence with distance, though it remains a descriptive pattern over 5 cells rather than a validated general law. The year-by-year estimates show no interpretable trend: confidence intervals are wide, particularly at the ends of the 2018–2023 series, and no claim is made about a link to the documented Adriatic oligotrophication trend — the data neither confirm nor rule it out. No formal refutation test was run for the causal forest, making this the most exploratory of the four causal analyses.
...
- **Unobserved confounding.** Refutation tests assess robustness to method and data, not the existence of confounders absent from the dataset (currents, other river inputs, solar radiation). If such confounders exist, part of the estimated effect may belong to them. This is the structural limit of observational causal inference.
- **Temporal autocorrelation.** Discharge and chlorophyll are autocorrelated series; standard confidence intervals are consequently too narrow. Point estimates are more reliable than their stated precision.
...
- **SST data coverage.** The nearest-pixel SST retrieval is valid for only 2 of the 5 cells across all six seasons in this reprocessed product; the other three consistently fall on a masked/land pixel, a systematic issue rather than missingness at random. Analyses that exclude SST as a confounder (A-bis, C) therefore use the full 5-cell sample, while those that include it (A, B) are estimated on a smaller effective sample.
...
Future work: integrate ARPAE-Daphne in-situ series for calibration and functional-group labels; extend the record to more seasons to strengthen both prediction and causal estimation; add heterogeneity/non-stationarity analysis (e.g. causal forests) to test whether the Po effect varies by coastal sector and has changed during the ongoing oligotrophication; and refine the demonstrative grid toward the official transects.
