...
The figures below (and in `docs/technical-report.md`) are generated by
`causal/plots.py`, which reuses each step's own fitted model - run it after
`make causal` to refresh them: `python causal/plots.py`.
...
![Causal effect estimates across methods (apparent, Step A, Step C, Step B classical and clustered SE), with 95% CI](../docs/figures/causal_effects_comparison.png)
...
![Spatial heterogeneity: estimated Po effect vs distance to the Po mouth, 90% CI](../docs/figures/spatial_heterogeneity.png)
...
![Temporal heterogeneity: estimated Po effect by year, 90% CI](../docs/figures/temporal_heterogeneity.png)
